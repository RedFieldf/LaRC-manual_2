...
- **影響期間**: **2026-06-16 頃から**（`@babel/standalone` 8.0.0 公開時）〜 修正まで
...
未固定URL（`latest` タグ）が Babel 8 を返し始めた時点が障害発生時点。

| バージョン | 公開日時 (UTC) |
|---|---|
| 7.26.4 | 2024-12-05 |
| **8.0.0** | **2026-06-16** ← ここから白画面化 |
| 8.0.1 | 2026-06-17 |
| 8.0.2 | 2026-06-18（調査時にCDNが返した版） |

→ **2026-06-16 の Babel 8.0.0 公開を境に発生**。それ以前（Babel 7 系の間）は正常稼働していた。
...
補足: Firestore 上の予約データの最新は 2025-12-02 だが、これは単に最後に入力された予約の日付であり、本障害（2026-06-16〜）とは無関係。
